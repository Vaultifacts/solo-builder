"""
utils/repo_index.py
RepoIndex — builds and loads a structural index of the repository for risk assessment.
...
The index is persisted to ``repo_index.json`` for fast reloading.
"""
...
class RepoIndex:
    ...
    def file_risk(self, filepath: str) -> int:
        """
        Compute a risk score for a file based on repo structure metrics.
    ...
    def subtask_risk(self, description: str) -> int:
        """
        Compute aggregate risk score for a subtask by matching file
    ...
    def _count_authors(self, fpath: str) -> int:
        """Count distinct git authors who have committed to this file."""
        try:
            r = subprocess.run(
    ...
    def _git_available(self) -> bool:
        """Check if git is available in the repo."""
        try:
            r = subprocess.run(
